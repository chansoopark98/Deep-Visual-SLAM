...
class ResBlock(tf.keras.layers.Layer):
    def __init__(self, filters, norm_type='group', strides=1, **kwargs):
        ...
        self.conv1 = tf.keras.layers.Conv2D(filters, 3, strides, 'same')
...
class BottleneckBlock(tf.keras.layers.Layer):
    def __init__(self, filters, norm_type='group', strides=1, **kwargs):
        ...
        self.conv1 = tf.keras.layers.Conv2D(filters//4, 1)
        self.conv2 = tf.keras.layers.Conv2D(filters//4, 3, strides, 'same')
...
class BasicEncoder(tf.keras.layers.Layer):
    def __init__(self, output_dim=128, norm_type='batch', drop_rate=0.0, **kwargs):
        ...
        self.conv1 = tf.keras.layers.Conv2D(64, 7, 2, 'same')
...
class SmallEncoder(tf.keras.layers.Layer):
    def __init__(self, output_dim=128, norm_type='batch', drop_rate=0.0, **kwargs):
        ...
        self.conv1 = tf.keras.layers.Conv2D(32, 7, 2, 'same')
        self.norm1 = Normalization(norm_type, groups=8)
