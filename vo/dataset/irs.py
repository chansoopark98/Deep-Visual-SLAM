...
class IrsMonoDataset(MonoDataset):
    def __init__(self,
                 config,
                 fold: str = 'train',
                 shuffle: bool = True,
    ...
    def _process_mono_scene(self, fold_dir: str):
        ...
        for scene_dir in scene_dirs:
            if os.path.isdir(scene_dir):
                ...
                intrinsic = self._load_calibration()
                ...
                dataset_dict = self._create_mono_samples(left_files, intrinsic)
...
class IrsStereoDataset(StereoDataset):
    def __init__(self,
                 config,
                 fold: str = 'train',
                 shuffle: bool = True,
    ...
    def _process_stereo_scene(self, fold_dir: str):
        ...
        for scene_dir in scene_dirs:
            ...
            if len(left_files) != len(right_files):
                print(f"Warning: Mismatch in {scene_dir} - Left: {len(left_files)}, Right: {len(right_files)}")
                continue

            left_K, right_K, stereo_T_L2R, stereo_T_R2L, baseline_m = self._load_stereo_calibration()
...
if __name__ == '__main__':
    ...
    # train_data = IrsMonoDataset(config=config, fold='train', is_train=True, augment=True)
    # valid_data = IrsMonoDataset(config=config, fold='valid', is_train=False, augment=False)
